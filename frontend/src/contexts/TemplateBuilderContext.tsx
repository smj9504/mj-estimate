...
export const TemplateBuilderProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    if (mode === 'edit' && template) {
      setState(prev => ({
        ...
        builderItems: template.template_items?.map((ti, idx) => ({
          ...
          rate: ti.line_item?.untaxed_unit_price || 0,
          quantity_multiplier: ti.quantity_multiplier,
          order_index: idx,
        })) || [],
  ...
  const saveSectionAsNewTemplate = useCallback(async (
    ...
    try {
      // Filter items with line_item_id - items should have this from invoice/estimate items
      const validItems = items.filter(item => {
        // Check if item has line_item_id directly or in source data
        const hasLineItemId = item.line_item_id || (item as any).line_item_id;
        if (!hasLineItemId) {
          console.warn('Item missing line_item_id:', item);
        }
        return hasLineItemId;
      });

      if (validItems.length === 0) {
        message.warning('No items with library references found. Please ensure items are linked to the line items library.');
        return null;
      }
      ...
      const templateData = {
        ...
        line_item_ids: validItems.map((item, index) => ({
  ...
  const saveTemplate = useCallback(async (companyId?: string): Promise<LineItemTemplate | null> => {
    try {
      const validItems = state.builderItems.filter(item => item.line_item_id);

      if (validItems.length === 0) {
        message.error('No items with library references to save');
        return null;
      }
      ...
      const templateData = {
        ...
        line_item_ids: validItems.map(item => ({
