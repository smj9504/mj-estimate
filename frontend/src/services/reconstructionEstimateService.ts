...
export const debrisCalculationAPI = {
  calculateQuick: async (request: QuickCalculationRequest): Promise<DebrisCalculationResult> => {
    const response = await axios.post(`${API_BASE_URL}/debris/calculate`, request);
    return response.data;
  },

  create: async (data: DebrisCalculationInput): Promise<DebrisCalculation> => {
    const response = await axios.post(`${API_BASE_URL}/debris/calculations`, data);
    return response.data;
  },
  ...
  getAll: async (params?: {
    ...
    const response = await axios.get(`${API_BASE_URL}/debris/calculations`, { params });
    return response.data;
  },

  getById: async (id: string): Promise<DebrisCalculation> => {
    const response = await axios.get(`${API_BASE_URL}/debris/calculations/${id}`);
    return response.data;
  },

  update: async (id: string, data: Partial<DebrisCalculationInput>): Promise<DebrisCalculation> => {
    const response = await axios.put(`${API_BASE_URL}/debris/calculations/${id}`, data);
    return response.data;
  },

  delete: async (id: string): Promise<void> => {
    await axios.delete(`${API_BASE_URL}/debris/calculations/${id}`);
  },
};
