...
def upgrade() -> None:
    ...
    webhook_events_exists = result.scalar()

    # Create webhook_events table only if it doesn't exist
    if not webhook_events_exists:
        op.create_table(
            'webhook_events',
        ...
        op.create_index(
            'ix_webhook_events_id',
            'webhook_events',
            ['id']
        )
        op.create_index(
            'ix_webhook_service_type',
            'webhook_events',
    ...
    # Check if companycam_photos table exists
    result = conn.execute(sa.text(
        "SELECT EXISTS (SELECT 1 FROM information_schema.tables "
        "WHERE table_name = 'companycam_photos')"
    ))
    companycam_photos_exists = result.scalar()

    # Create companycam_photos table only if it doesn't exist
    if not companycam_photos_exists:
        op.create_table(
            'companycam_photos',
            sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True),
            sa.Column(
        ...
        op.create_index(
            'ix_companycam_photos_id',
            'companycam_photos',
            ['id']
        )
        op.create_index(
            'ix_companycam_photo_job',
            'companycam_photos',
            ['water_mitigation_job_id']
        )
        op.create_index(
            'ix_companycam_photo_external',
            'companycam_photos',
            ['companycam_photo_id']
        )
        op.create_index(
            'ix_companycam_project_id',
            'companycam_photos',
            ['companycam_project_id']
        )
