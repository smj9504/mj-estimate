...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('training_image_id', sa.String(length=36), nullable=False),
        sa.Column('material_order', sa.Integer(), nullable=True, server_default='1'),
        ...
        sa.ForeignKeyConstraint(['training_image_id'], ['training_images.id'], ),
        sa.PrimaryKeyConstraint('id')
    )

    # Create indexes
    op.create_index('idx_detected_materials_image_id', 'detected_materials', ['training_image_id'], unique=False)
    op.create_index('idx_detected_materials_category', 'detected_materials', ['category'], unique=False)


def downgrade() -> None:
    # Drop indexes
    op.drop_index('idx_detected_materials_category', table_name='detected_materials')
    op.drop_index('idx_detected_materials_image_id', table_name='detected_materials')
